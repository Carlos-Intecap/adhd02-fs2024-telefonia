<!doctype html>
<html lang="es">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Telefonía</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
</head>

<body class="bg-body-secondary">

    <div class="container">
        
        <div class="row justify-content-center m-4">
            <div class="col col-4 text-center">
                <h1>Base de datos - Telefonía</h1>
                <div class="list-group">
                    <a href="clientes"
                        class="list-group-item list-group-item-action list-group-item-secondary">Clientes</a>
                    <a href="lineas" class="list-group-item list-group-item-action list-group-item-secondary">Líneas
                        Telefónicas</a>
                    <a href="planes" class="list-group-item list-group-item-action list-group-item-secondary">Planes</a>
                </div>
            </div>
        </div>
    </div>
    <div class="card-group text-center container">
            <div class="card">
                <div class="card-body">
                  <i class="bi bi-person-fill fs-1"></i>
                  <h5 class="card-title">Clientes</h5>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                  <i class="bi bi-telephone-fill fs-1"></i>
                  <h5 class="card-title">Líneas telefónicas</h5>
                </div>
            </div>
            <div class="card">
                <div class="card-body">
                <i class="bi bi-menu-button-wide fs-1"></i>
                    <h5 class="card-title">Planes</h5>
                </div>
            </div>
        </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous">
    </script>
</body>

</html>